...
import {
  Alert, Box, Chip, CircularProgress, MenuItem, Paper, Select,
...
const ENTITY_OPTIONS = ["", "Student", "Doctor", "Admin", "Exam", "Grade", "Enrollment", "Subject", "College", "Department"];
const ACTION_OPTIONS = ["", "Create", "Update", "Delete", "Restore", "Login", "Logout"];
  ...
  return (
    <div className="space-y-4">
      ...
      <Box sx={{ display: "flex", gap: 2, flexWrap: "wrap" }}>
        <Select size="small" value={entityFilter} onChange={(e) => { setEntityFilter(e.target.value); setPage(1); }}
          displayEmpty sx={{ minWidth: 150 }}>
          <MenuItem value="">All Entities</MenuItem>
          {ENTITY_OPTIONS.filter(Boolean).map((e) => <MenuItem key={e} value={e}>{e}</MenuItem>)}
        </Select>
        <Select size="small" value={actionFilter} onChange={(e) => { setActionFilter(e.target.value); setPage(1); }}
          displayEmpty sx={{ minWidth: 140 }}>
          <MenuItem value="">All Actions</MenuItem>
          {ACTION_OPTIONS.filter(Boolean).map((a) => <MenuItem key={a} value={a}>{a}</MenuItem>)}
        </Select>
        <TextField
          size="small"
              ...
              <TableBody>
                {logs.map((log) => (
                  <TableRow key={log.id} hover>
                    <TableCell>
                      <Chip label={log.actionType} size="small" color={actionColor(log.actionType)} />
                    </TableCell>
                    <TableCell>
